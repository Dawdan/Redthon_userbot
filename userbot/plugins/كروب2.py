...
async def get_users(event):
    ...
    Redthon = await get_chatinfo(event)
    chat = await event.get_chat()
    if event.is_private:
    ...
    async for user in event.client.iter_participants(Redthon.full_chat.id):
        try:
...
async def _(event):
    ...
    Redthonevent = await edit_or_reply(event, "**❈╎جـاري اتمـام العمليـه .. الرجـاء الانتظـار 🚸**")
    ...
    async for user in event.client.iter_participants(event.chat_id):
        ...
        try:
            ...
            LOGS.info(str(e))
    await Redthonevent.edit(f"**❈╎تـم  تفلـيش {total} عضـو .. بنجـاح ☑️🗑**")
    
async def ban_user(chat_id, i, rights):
    try:
        await Redthon(functions.channels.EditBannedRequest(chat_id, i, rights))
        return True, None
    except Exception as exc:
        return False, str(exc)
