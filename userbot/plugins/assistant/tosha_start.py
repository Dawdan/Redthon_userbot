...
# =================== OWNER - MSS3G =================== #
ALIVE_NAME = Config.ALIVE_NAME if Config.ALIVE_NAME else "@Redthon"
    ...
    if LOAD_MYBOT == "False":
        ...
        if BOT_PIC:
            await tgbot.send_message(event.chat_id, BOT_PIC,
                                  ...
                                  buttons=[
                                      [Button.inline(
                                          "⚜️ 𝐑𝐀𝐈𝐈𝐒𝙏𝙃𝙊𝙉 - 𝙐𝙎𝙀𝙍𝘽𝙊𝙏 ⁦⚜️", data="Redthon")]
                                  ])
        else:
...
async def logs(event):
    ...
    await tgbot.send_file(
        ...
        "logs.txt",
        reply_to=event.id,
        caption="**Heroku** Redthon Logs",
        buttons=[
            [Button.url("💠 𝙑𝙄𝙀𝙒 𝙊𝙉𝙇𝙄𝙉𝙀 💠", f"{url}")],
            [Button.url("💠 𝘾𝙍𝘼𝙎𝙃𝙀𝘿 💠", "t.me/Redthon")]
        ])
...
@tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"Redthon"))
          )  # pylint: disable=oof
async def settings(event):
    await event.delete()
...
async def settings(event):
    await event.edit("** ⌔∮ هنا ستجد رابط ريس ثـون ورابط التنصيب ايضا**:",
                     buttons=[
                         [Button.url("🔗 رابط السورس🔗", url="https://github.com/Redthon/Redthon_USERBOT")],
                         [Button.url("🔗 رابط التنصيب 🔗", url="https://t.me/Redthon/105")],
                         [Button.url("✨ الدعم ✨",
                                     url="https://t.me/Redthon")]
                     ])
...
async def settings(event):
    if event.sender_id == OWNER_ID:
        ...
        await event.answer("انت لا تستطيع استخدام البوت احصل على بوتك من @Redthon", alert=True)


# Redthon zag - زغرفه ريسثون
