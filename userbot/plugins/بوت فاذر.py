...
async def _(event):
    ...
    if tgbotusername is not None:
        try:
            results = await event.client.inline_query(tgbotusername, "MSS3G_Ahmed")
            await results[0].click(
                event.chat_id, reply_to=event.reply_to_msg_id, hide_via=True
            )
            await event.delete()
        except:
            MSS3G = await eor(event, "**❈╎جـارِ الاتصـال ببـوت فـاذر ...⏣**\n**❈╎تـم تفعيـل انـلاين بـوتك .. بنجـاح ☑️**\n\n**❈╎جـاري اعـادة تشغيل البـوت الرجـاء الانتظـار  ▬▭...𓅫**")
            async with bot.conversation(chat) as conv:
                try:
                    ...
                    fifth = await conv.send_message("Redthon")
                    sixth = await conv.get_response()
                    seventh = await conv.send_message(perf)
                    eighth = await conv.get_response()
                    await bot.send_read_acknowledge(conv.chat_id)
                except YouBlockedUserError:
                    return await MSS3G.edit("**❈╎قـم بإلغـاء الحظـر عـن بـوت فـاذر اولا @Botfather .. ثـم اعـد المحـاوله**")
