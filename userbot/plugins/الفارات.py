...
#
# This file is a part of < https://github.com/raiis1/Redthon_USERBOT/ >
# PLease read the GNU Affero General Public License in
# <https://www.github.com/raiis1/Redthon_USERBOT/blob/master/LICENSE/>.
...
admin_cmd = (
    "𓆩 [𝗦𝗼𝘂𝗿𝗰𝗲 𝗥𝗲𝗱𝘁𝗵𝗼𝗻 𝗖𝗼𝗻𝗳𝗶𝗴 𝗩𝗮𝗿𝘀 - اوامـر الفـارات](t.me/Redthon) 𓆪\n\n"
...
async def variable(event):
    ...
    Redthon = await edit_or_reply(event, "**✾╎جـاري اضـافة الفـار الـى بـوتك ...**")
    # All Rights Reserved for "Redthon - UserBot" "حنــــش"
    if input_str == "كليشة الفحص" or input_str == "كليشه الفحص":
        variable = "Redthon_MEDIA"
        await asyncio.sleep(1.5)
        if variable in heroku_var:
            await Redthon.edit("**✾╎تم تغييـر {} بنجـاح ☑️**\n**✾╎المتغير : ↶**\n `{}` \n**✾╎يتم الان اعـادة تشغيـل بـوت زد ثـون يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str, vinfo))
        else:
            await Redthon.edit("**✾╎تم اضافـة {} بنجـاح ☑️** \n**✾╎المضاف اليه :**\n `{}` \n**✾╎يتم الان اعـادة تشغيـل بـوت زد ثـون يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str, vinfo))
        ...
        variable = "CUSTOM_ALIVE_EMRedthon"
        await asyncio.sleep(1.5)
        if variable in heroku_var:
            await Redthon.edit("**✾╎تم تغييـر {} بنجـاح ☑️**\n**✾╎المتغيـر : ↶**\n `{}` \n**✾╎يتم الان اعـادة تشغيـل بـوت زد ثـون يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str, vinfo))
        else:
        ...
        await asyncio.sleep(1.5)
        if variable in heroku_var:
            await Redthon.edit("**✾╎تم تغيـر المنطقـة الزمنيـه**\n **✾╎المتغير : دولـة مصـر 🇪🇬**\n\n**✾╎يتم الان اعـادة تشغيـل بـوت زد ثـون يستغـرق الامر 2-1 دقيقـه ▬▭ ...**")
        else:
            await Redthon.edit("**✾╎تم اعـادة تغيـر المنطقـة الزمنيـه**\n **✾╎المتغير : دولـة مصـر 🇪🇬**\n\n**✾╎يتم الان اعـادة تشغيـل بـوت زد ثـون يستغـرق الامر 2-1 دقيقـه ▬▭ ...**")
        heroku_var[variable] = vinfo
        ...
        if input_str:
            return await Redthon.edit("**✾╎عـذراً .. لايوجـد هنالك فـار بإسـم {} ؟!..**".format(input_str))
...
async def variable(event):
    ...
    heroku_var = app.config()
    Redthon = await edit_or_reply(event, "**✾╎جـاري حـذف الفـار مـن بـوتك 🚮...**")
    # All Rights Reserved for "Redthon - UserBot" "حنــــش"
    if input_str == "كليشة الفحص" or input_str == "كليشه الفحص":
        variable = "Redthon_MEDIA"
        await asyncio.sleep(1.5)
        if variable not in heroku_var:
        	return await Redthon.edit("**✾╎عـذراً .. فـار {} غير موجود اصـلاً...**".format(input_str))

        await Redthon.edit("**✾╎تم حـذف {} بنجـاح ☑️**\n**✾╎المتغيـر المحـذوف : ↶**\n `{}` \n**✾╎يتم الان اعـادة تشغيـل بـوت زد ثـون يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str, heroku_var[variable]))
...
async def variable(event):
    ...
    input_str = event.text[5:]
    heroku_var = app.config()
    Redthon = await edit_or_reply(event, "**✾╎جـاري جلـب الفـار مـن بـوتك 🛂...**")
    # All Rights Reserved for "Redthon - UserBot" "حنــــش"
    if input_str == "رمز الوقتي" or input_str == "رمز الاسم الوقتي":
        variable = "CUSTOM_ALIVE_EMRedthon"
        await asyncio.sleep(1.5)
        if variable in heroku_var:
            await Redthon.edit("**✾╎الفـار {} موجـود ☑️**\n**✾╎المتغيـر : ↶**\n `{}` \n**✾╎قنـاة السـورس : @Redthon**".format(input_str, heroku_var[variable]))
        else:
            await Redthon.edit("**✾╎ الفـار {} غيـر موجـود ❌** \n**✾╎المتغيـر :**\n `{}` \n**✾╎قنـاة السـورس : @Redthon**".format(input_str, heroku_var[variable]))
        ...
        await asyncio.sleep(1.5)
        if variable in heroku_var:
            await Redthon.edit("**✾╎المطـور {} موجـود 🧑🏻‍💻☑️**\n**✾╎ايدي المطـور : ↶**\n `{}` \n**✾╎قنـاة السـورس : @Redthon**".format(input_str, heroku_var[variable]))
        else:
            await Redthon.edit("**✾╎ المطـور {} غيـر موجـود 🧑🏻‍💻❌** \n**✾╎المتغيـر :**\n `{}` \n**✾╎قنـاة السـورس : @Redthon**".format(input_str, heroku_var[variable]))
...
async def variable(event):
    ...
    vmsr = "Africa/Cairo"
    heroku_var = app.config()
    Redthon = await edit_or_reply(event, "**✾╎جـاري أعـداد المنطقـه الزمنيـه لـ زدثــون 🌐...**")
    # All Rights Reserved for "Redthon - UserBot" "حنــــش"
    if input_str == "العراق" or input_str == "عراق":
        variable = "TZ"
        await asyncio.sleep(1.5)
        if variable in heroku_var:
            await Redthon.edit("**✾╎تم تغييـر المنطقـه الزمنيـه .. بنجـاح ☑️**\n**✾╎المتغير : ↶**\n دولـة `{}` 🇮🇶 \n**✾╎يتم الان اعـادة تشغيـل بـوت زد ثـون يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str))
        else:
            await Redthon.edit("**✾╎تم اضـافـة المنطقـه الزمنيـه .. بنجـاح ☑️**\n**✾╎المضـاف اليـه : ↶**\n دولـة `{}` 🇮🇶 \n**✾╎يتم الان اعـادة تشغيـل بـوت زد ثـون يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str))
...
async def zelzalll(event):
    ...
    if not previous_message:
        await edit_or_reply(event, "**حنــــش**")
        return
    user_id = replied_user.user.id
    heroku_var = app.config()
    Redthon = await edit_or_reply(event, "**✾╎جـاري رفـع الشخـص مطـور اسـاسي معـك فـي البـوت 🧑🏻‍💻...**")
    # All Rights Reserved for "Redthon - UserBot" "حنــــش"
    variable = "SUDO_USERS"
    await asyncio.sleep(1.5)
    if variable in heroku_var:
        await Redthon.edit("**✾╎تـم رفـع**  [الشخـص🧑🏻‍💻](tg://user?id={})  **مطـور .. بنجـاح ☑️**\n**✾╎بصـلاحيات تحـكم كـامل بالاوامـر 🦾**\n\n**✾╎يتم الان اعـادة تشغيـل بـوت زد ثـون يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(user_id))
    else:
        await Redthon.edit("**✾╎تـم إعـادة رفـع**  [الشخـص🧑🏻‍💻](tg://user?id={})  **مطـور .. بنجـاح ☑️**\n**✾╎بصـلاحيات تحـكم كـامل بالاوامـر 🦾**\n\n**✾╎يتم الان اعـادة تشغيـل بـوت زد ثـون يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(user_id))
...
async def zelzalll(event):
    ...
    user_id = replied_user.user.id
    heroku_var = app.config()
    Redthon = await edit_or_reply(event, "**✾╎جـاري تنزيـل الشخـص من مطـورين البـوت 🛗...**")
    # All Rights Reserved for "Redthon - UserBot" "حنــــش"
    if previous_message:
	    variable = "SUDO_USERS"
	    await asyncio.sleep(1.5)
	    if variable not in heroku_var:
	        return await Redthon.edit(f"**✾╎عـذراً .. فـار {input_str} غير موجود اصـلاً...**")
	
	    await Redthon.edit("**✾╎تـم تنزيـل**  [الشخـص🧑🏻‍💻](tg://user?id={})  **مطـور .. بنجـاح ☑️**\n**✾╎لـم يعد لديـه تحـكم بأوامـر البـوت 🥲**\n\n**✾╎يتم الان اعـادة تشغيـل بـوت زد ثـون يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(user_id))
	    del heroku_var[variable]
...
async def _(tosh):
    ...
    heroku_var = app.config()
    Redthon = await eor(tosh, "**✾╎جـاري اضـافة فـار الصـورة الـى بـوتك ...**")
    ...
    if tosh.reply_to_msg_id:
        ...
        if input_str in ["الحماية", "الحمايه"]:
            ...
            try:
                ...
                await Redthon.edit("**✾╎خطا : **" + str(exc))
                ...
                await Redthon.edit("**✾╎تم تغييـر صـورة {} .. بنجـاح ☑️**\n**✾╎المتغيـر : ↶**\n `{}` \n**✾╎يتم الان اعـادة تشغيـل بـوت زد ثـون يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str, vinfo))
