# whois code for Redthon edit by ~ @Redthon
...
Redthon_TEXT = Config.CUSTOM_ALIVE_TEXT or "╮•⎚ مـعلومات الـشخص مـن بـوت ريسثـون"
RedthonM = Config.CUSTOM_ALIVE_EMOJI or " •❃ "
...
async def fetch_info(replied_user, event):
    ...
    caption = f"<b> {Redthon_TEXT} </b>\n"
    caption += f"<b> ٴ•━─━─━─━─━─━─━─━─━• </b>\n"
    caption += f"<b> {RedthonM}| الاسـم    ⇦ </b> {first_name} {last_name}\n"
    caption += f"<b> {RedthonM}| المعـرف  ⇦ </b> {username}\n"
    caption += f"<b> {RedthonM}| الايـدي   ⇦ </b> <code>{user_id}</code>\n"
    caption += f"<b> {RedthonM}| الرتبـــه  ⇦ {rotbat} </b>\n"
    caption += f"<b> {RedthonM}| الصـور   ⇦ </b> {replied_user_profile_photos_count}\n"
    caption += f"<b> {RedthonM}|الحسـاب ⇦ </b> "
    caption += f'<a href="tg://user?id={user_id}">{first_name}</a>'
    caption += f"\n<b> {RedthonM}| الـمجموعات المشتـركة ⇦ </b> {common_chat} \n"
    caption += f"<b> {RedthonM}| البايـو    ⇦ </b> {user_bio} \n"
    caption += f"<b> ٴ•━─━─━─━─━─━─━─━─━• </b>\n"
    caption += f"<b> 𓆩 𝐒𝐨𝐮𝐫𝐜𝐞 𝐑𝐞𝐝𝐭𝐡𝐨𝐧 𓆪 </b> - @Redthon "
    return photo, caption


@bot.on(
    Redthon_cmd(pattern="رابط الحساب(?: |$)(.*)")
)
@bot.on(
