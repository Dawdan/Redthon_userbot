...
@Redthon.on(QQ070_cmd(pattern="طقس (.*)"))
@Redthon.on(sudo_cmd(pattern="طقس (.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    Red = "adf0cf22618186fc11e9f89c090cb356"
    sample_url = "https://api.openweathermap.org/data/2.5/weather?q={}&appid={}"
    input_str = event.pattern_match.group(1)
    async with aiohttp.ClientSession() as session:
        response_api_zero = await session.get(sample_url.format(input_str, Red))
...
@Redthon.on(QQ070_cmd(pattern="يكتب(?: |$)(.*)"))
async def _(event):
...
@Redthon.on(QQ070_cmd(pattern="صوتيه(?: |$)(.*)"))
async def _(event):
    t = event.pattern_match.group(1)
    if not (t or t.isdigit()):
        t = 100
...
@Redthon.on(QQ070_cmd(pattern="فيد(?: |$)(.*)"))
async def _(event):
    t = event.pattern_match.group(1)
    if not (t or t.isdigit()):
        t = 100
...
@Redthon.on(QQ070_cmd(pattern="لعبه(?: |$)(.*)"))
async def _(event):
    t = event.pattern_match.group(1)
    if not (t or t.isdigit()):
        t = 100
