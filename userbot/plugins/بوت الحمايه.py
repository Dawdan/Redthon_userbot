...
# All Rights Reserved for "Redthon - UserBot" "ردثــــون"
ANTI_DDDD_Redthon_MODE = ChatBannedRights(
    until_date=None, view_messages=None, send_media=True, send_stickers=True, send_gifs=True
)


@Redthon.on(admin_cmd(pattern=r"قفل (.*)"))
@Redthon.on(sudo_cmd(pattern=r"قفل (.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    input_str = event.pattern_match.group(1)
    Redthon_id = event.chat_id
    # All Rights Reserved for "Redthon - UserBot" "ردثــــون"
    if not event.is_group:
        return await edit_delete(event, "**ايا مطـي! ، هـذه ليست مجموعـة لقفـل الأشيـاء**")
    chat_per = (await event.get_chat()).default_banned_rights
    if input_str == "البوتات":
        update_lock(Redthon_id, "bots", True)
        return await edit_or_reply(event, "**❈╎تـم قفـل {} بنجـاح ✅ •**\n\n**❈╎خاصيـة الطـرد والتحذيـر •**".format(input_str))
    if input_str == "المعرفات":
        update_lock(Redthon_id, "button", True)
        return await edit_or_reply(event, "**❈╎تـم قفـل {} بنجـاح ✅ •**\n\n**❈╎خاصيـة المسـح والتحذيـر •**".format(input_str))
    if input_str == "الدخول":
        update_lock(Redthon_id, "location", True)
        return await edit_or_reply(event, "**❈╎تـم قفـل {} بنجـاح ✅ •**\n\n**❈╎خاصيـة الطـرد والتحذيـر •**".format(input_str))
    if input_str == "الفارسيه":
        update_lock(Redthon_id, "egame", True)
        return await edit_or_reply(event, "**❈╎تـم قفـل {} بنجـاح ✅ •**\n\n**❈╎خاصيـة المسـح والتحذيـر •**".format(input_str))
    if input_str == "الاضافه":
        update_lock(Redthon_id, "contact", True)
        return await edit_or_reply(event, "**❈╎تـم قفـل {} بنجـاح ✅ •**\n\n**❈╎خاصيـة الطـرد والتحذيـر •**".format(input_str))
    if input_str == "التوجيه":
        update_lock(Redthon_id, "forward", True)
        return await edit_or_reply(event, "**❈╎تـم قفـل {} بنجـاح ✅ •**\n\n**❈╎خاصيـة المسـح والتحذيـر •**".format(input_str))
    if input_str == "الميديا":
        update_lock(Redthon_id, "game", True)
        return await edit_or_reply(event, "**❈╎تـم قفـل {} بنجـاح ✅ •**\n\n**❈╎خاصيـة المسـح بالتقييـد والتحذيـر •**".format(input_str))
    if input_str == "الانلاين":
        update_lock(Redthon_id, "inline", True)
        return await edit_or_reply(event, "**❈╎تـم قفـل {} بنجـاح ✅ •**\n\n**❈╎خاصيـة المسـح والتحذيـر •**".format(input_str))
    if input_str == "الفشار":
        update_lock(Redthon_id, "rtl", True)
        return await edit_or_reply(event, "**❈╎تـم قفـل {} بنجـاح ✅ •**\n\n**❈╎خاصيـة المسـح والتحذيـر •**".format(input_str))
    if input_str == "الروابط":
        update_lock(Redthon_id, "url", True)
        return await edit_or_reply(event, "**❈╎تـم قفـل {} بنجـاح ✅ •**\n\n**❈╎خاصيـة المسـح والتحذيـر •**".format(input_str))
    if input_str == "الكل":
        update_lock(Redthon_id, "bots", True)
        update_lock(Redthon_id, "location", True)
        ...
        update_lock(Redthon_id, "button", True)
        update_lock(Redthon_id, "inline", True)
        update_lock(Redthon_id, "video", True)
        update_lock(Redthon_id, "sticker", True)
        update_lock(Redthon_id, "voice", True)
...
@Redthon.on(admin_cmd(pattern="فتح (.*)"))
@Redthon.on(sudo_cmd(pattern="فتح (.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    input_str = event.pattern_match.group(1)
   ...
   # All Rights Reserved for "Redthon - UserBot" "ردثــــون"
    ...
    chat_per = (await event.get_chat()).default_banned_rights
    if input_str == "البوتات":
        update_lock(Redthon_id, "bots", False)
        return await edit_or_reply(event, "**❈╎تـم فتـح** {} **بنجـاح ✅ 𓆰•**".format(input_str))
    if input_str == "الدخول":
        update_lock(Redthon_id, "location", False)
        return await edit_or_reply(event, "**❈╎تـم فتـح** {} **بنجـاح ✅ 𓆰•**".format(input_str))
    if input_str == "الاضافه":
        update_lock(Redthon_id, "contact", False)
        return await edit_or_reply(event, "**❈╎تـم فتـح** {} **بنجـاح ✅ 𓆰•**".format(input_str))
    if input_str == "التوجيه":
        update_lock(Redthon_id, "forward", False)
        return await edit_or_reply(event, "**❈╎تـم فتـح** {} **بنجـاح ✅ 𓆰•**".format(input_str))
    if input_str == "الفارسيه":
        update_lock(Redthon_id, "egame", False)
        return await edit_or_reply(event, "**❈╎تـم فتـح** {} **بنجـاح ✅ 𓆰•**".format(input_str))
    if input_str == "الفشار":
        update_lock(Redthon_id, "rtl", False)
        return await edit_or_reply(event, "**❈╎تـم فتـح** {} **بنجـاح ✅ 𓆰•**".format(input_str))
    if input_str == "الروابط":
        update_lock(Redthon_id, "url", False)
        return await edit_or_reply(event, "**❈╎تـم فتـح** {} **بنجـاح ✅ 𓆰•**".format(input_str))
    if input_str == "الميديا":
        update_lock(Redthon_id, "game", False)
        return await edit_or_reply(event, "**❈╎تـم فتـح** {} **بنجـاح ✅ 𓆰•**".format(input_str))
    if input_str == "المعرفات":
        update_lock(Redthon_id, "button", False)
        return await edit_or_reply(event, "**❈╎تـم فتـح** {} **بنجـاح ✅ 𓆰•**".format(input_str))
    if input_str == "الانلاين":
        update_lock(Redthon_id, "inline", False)
        return await edit_or_reply(event, "**❈╎تـم فتـح** {} **بنجـاح ✅ 𓆰•**".format(input_str))
    if input_str == "الكل":
        update_lock(Redthon_id, "bots", False)
        update_lock(Redthon_id, "location", False)
        ...
        update_lock(Redthon_id, "button", False)
        update_lock(Redthon_id, "inline", False)
        update_lock(Redthon_id, "video", False)
        update_lock(Redthon_id, "sticker", False)
        update_lock(Redthon_id, "voice", False)
        return await edit_or_reply(event, "**❈╎تـم فتـح** {} **بنجـاح ✅ 𓆰•**".format(input_str))
    if input_str == "الفارسيه":
        update_lock(Redthon_id, "egame", False)
        return await edit_or_reply(event, "**❈╎تـم فتـح** {} **بنجـاح ✅ 𓆰•**".format(input_str))
...
@Redthon.on(admin_cmd(pattern="الحاله$"))
@Redthon.on(sudo_cmd(pattern="الحاله$", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
   # All Rights Reserved for "Redthon - UserBot" "ردثــــون"
    res = ""
    current_Redthon_locks = get_locks(event.chat_id)
    if not current_Redthon_locks:
        res = "**إعدادات الحمايه في هذه الدردشة**"
    else:
        res = "- فيمـا يلي إعـدادات الحمـاية في هـذه الدردشـة: \n"
        ubots = "❌" if current_Redthon_locks.bots else "✅"
        uegame = "❌" if current_Redthon_locks.egame else "✅"
        urtl = "❌" if current_Redthon_locks.rtl else "✅"
        uforward = "❌" if current_Redthon_locks.forward else "✅"
        ubutton = "❌" if current_Redthon_locks.button else "✅"
        uurl = "❌" if current_Redthon_locks.url else "✅"
        ugame = "❌" if current_Redthon_locks.game else "✅"
        ulocation = "❌" if current_Redthon_locks.location else "✅"
        ubutton = "❌" if current_Redthon_locks.button else "✅"
        uinline = "❌" if current_Redthon_locks.inline else "✅"
        res += f"👉 `البوتات`: `{ubots}`\n"
...
@Redthon.on(events.MessageEdited())
@Redthon.on(events.NewMessage())
    ...
    if is_locked(Redthon_id, "rtl") and ("خرا" in hhh or "كسها" in hhh or "كسمك" in hhh or "كسختك" in hhh or "عيري" in hhh or "كسخالتك" in hhh or "خرا بالله" in hhh or "عير بالله" in hhh or "كسخواتكم" in hhh or "اختك" in hhh or "بڪسسخخت" in hhh or "كحاب" in hhh or "مناويج" in hhh or "كحبه" in hhh or " كواد " in hhh or "كواده" in hhh or "تبياته" in hhh or "تبياتة" in hhh or "فرخ" in hhh or "كحبة" in hhh or "فروخ" in hhh or "طيز" in hhh or "آإيري" in hhh or "اختج" in hhh or "سالب" in hhh or "موجب" in hhh or "فحل" in hhh or "كسي" in hhh or "كسك" in hhh or "كسج" in hhh or "مكوم" in hhh or "نيج" in hhh or "نتنايج" in hhh or "مقاطع" in hhh or "ديوث" in hhh or "دياث" in hhh or "اديث" in hhh or "محارم" in hhh or "سكس" in hhh or "مصي" in hhh or "اعرب" in hhh or "أعرب" in hhh or "قحب" in hhh or "قحاب" in hhh or "عراب" in hhh or "مكود" in hhh or "عربك" in hhh or "مخنث" in hhh or "مخنوث" in hhh or "فتال" in hhh or "زاني" in hhh or "زنا" in hhh or "لقيط" in hhh or "بنات شوارع" in hhh or "بنت شوارع" in hhh or "نيك" in hhh or "منيوك" in hhh or "منيوج" in hhh or "نايك" in hhh or "قواد" in hhh or "زب" in hhh or "اير" in hhh or "ممحو" in hhh or "بنت شارع" in hhh or " است " in hhh or "اسات" in hhh or "زوب" in hhh or "عيير" in hhh or "املس" in hhh or "مربرب" in hhh or " خول " in hhh or "عرص" in hhh or "قواد" in hhh or "اهلاتك" in hhh or "جلخ" in hhh or "ورع" in hhh or "شرمو" in hhh or "فرك" in hhh or "رهط" in hhh):
        if admin == malath or Redthon.is_admin or admin in Redthon_dev:
	        ...
	        try:
	            ...
	            update_lock(Redthon_id, "rtl", False)
    if is_locked(Redthon_id, "game") and event.message.media:
        if admin == malath or Redthon.is_admin or admin in Redthon_dev:
            return
        else:
	        try:
	            ...
	            update_lock(Redthon_id, "game", False)
    if is_locked(Redthon_id, "forward") and event.fwd_from:
        if admin == malath or Redthon.is_admin or admin in Redthon_dev:
            return
        else:
	        try:
	            ...
	            update_lock(Redthon_id, "forward", False)
    if is_locked(Redthon_id, "button") and "@" in hhh:
        if admin == malath or Redthon.is_admin or admin in Redthon_dev:
            return
        else:
	        try:
    ...
    if is_locked(Redthon_id, "egame") and ("فارسى" in hhh or "خوببی" in hhh or "میخوام" in hhh or "کی" in hhh or "پی" in hhh or "گ" in hhh or "خسته" in hhh or "صكص" in hhh or "راحتی" in hhh or "بیام" in hhh or "بپوشم" in hhh or "گرمه" in hhh or "چ" in hhh or "چه" in hhh or "ڬ" in hhh or "ٺ" in hhh or "چ" in hhh or "ڿ" in hhh or "ڇ" in hhh or "ڀ" in hhh or "ڎ" in hhh or "ݫ" in hhh or "ژ" in hhh or "ڟ" in hhh or "۴" in hhh or "ريسن" in hhh or "دخترا" in hhh or "كسى" in hhh or "مک" in hhh or "خالى" in hhh or "ݜ" in hhh or "ڸ" in hhh or "پ" in hhh or "بند" in hhh or "عزيزم" in hhh or "برادر" in hhh or "باشى" in hhh or "ميخوام" in hhh or "خوبى" in hhh or "ميدم" in hhh or "كى اومدى" in hhh or "خوابيدين" in hhh):
        if admin == malath or Redthon.is_admin or admin in Redthon_dev:
	        ...
	        try:
	            ...
	            update_lock(Redthon_id, "egame", False)
    if is_locked(Redthon_id, "url") and "http" in hhh:
        if admin == malath or Redthon.is_admin or admin in Redthon_dev:
            return
        else:
	        try:
	            ...
	            update_lock(Redthon_id, "url", False)
    if is_locked(Redthon_id, "inline") and event.message.via_bot:
        if admin == malath or Redthon.is_admin or admin in Redthon_dev:
            return
        else:
	        try:
...
# Copyright (C) 2022 Redthon
@Redthon.on(events.ChatAction())
async def _(event):
    if not event.is_private:
        chat = await event.get_chat()
        admin = chat.admin_rights
    ...
    if event.user_added:
        ...
        Redthon = await bot.get_permissions(event.chat_id, admin_by)
        is_ban_able = False
        ...
        for user_id in added_users:
            ...
            if event.user_added:
                is_ban_able = True
                if admin_by == malath or Redthon.is_admin or admin_by in Redthon_dev:
                    return
                else:
	                try:
	                    await event.client(
...
async def _(event):
    ...
    if event.user_joined: 
        Redthony = await event.client.get_entity(event.user_id)
        is_ban_able = False
        rights = types.ChatBannedRights(until_date=None, view_messages=True)
        if event.user_joined:
            is_ban_able = True
            if Redthony.id in Redthon_dev:
                return
            else:
	            try:
	                await event.client(
	                        functions.channels.EditBannedRequest(
	                            event.chat_id, Redthony.id, rights
	                        )
	                    )
	                await event.reply(
...
async def _(event):
    ...
    if event.user_added:
        ...
        for user_id in added_users:
            ...
            if user_obj.bot:
                is_ban_able = True
                if admin_by == malath or admin_by in Redthon_dev:
                    return
                else:
	                try:
	                    await event.client(
...
# Copyright (C) 2022 Redthon
@Redthon.on(admin_cmd(pattern=f"البوتات ?(.*)"))
@Redthon.on(sudo_cmd(pattern="البوتات ?(.*)", allow_sudo=True))
async def admin(Redthon):
    con = Redthon.pattern_match.group(1).lower()
    del_u = 0
    del_status = "**❈╎مجمـوعتك/قناتـك في أمـان ✅.. لاتوجـد بوتـات في هذه الدردشـة ༗**"
    if con != "طرد":
        event = await edit_or_reply(Redthon, "**❈╎جـاري البحـث عن بوتات في هـذه الدردشـة ...🝰**")
        async for user in Redthon.client.iter_participants(Redthon.chat_id):
            if user.bot:
                del_u += 1
                await sleep(0.5)
        if del_u > 0:
            del_status = f"🛂**┊كشـف البـوتات -** 𝐒𝐨𝐮𝐫𝐜𝐞 𝐑𝐞𝐝𝐭𝐡𝐨𝐧\
                           \n\n**❈╎تم العثور على** **{del_u}**  **بـوت**\
                           \n**❈╎لطـرد البوتات استخدم الامـر التالي ⩥** `.البوتات طرد`"
        await event.edit(del_status)
        return
    # All Rights Reserved for "Redthon - UserBot" "ردثــــون"
    chat = await Redthon.get_chat()
    admin = chat.admin_rights
    creator = chat.creator
    if not admin and not creator:
        await edit_delete(Redthon, "**❈╎عـذراً .. احتـاج الى صلاحيـات المشـرف هنـا**", 5)
        return
    event = await edit_or_reply(Redthon, "**❈╎جـارِ طـرد البوتـات من هنـا ...⅏**")
    del_u = 0
    del_a = 0
    async for user in Redthon.client.iter_participants(Redthon.chat_id):
        if user.bot:
            try:
                await Redthon.client.kick_participant(Redthon.chat_id, user.id)
                await sleep(0.5)
                del_u += 1
    ...
    if del_a > 0:
        del_status = f"❇️**┊طـرد البـوتات -** 𝐒𝐨𝐮𝐫𝐜𝐞 𝐑𝐞𝐝𝐭𝐡𝐨𝐧\
    ...
    if BOTLOG:
        await Redthon.client.send_message(
            BOTLOG_CHATID,
            f"#طـرد_البوتـات\
            \n ❈╎{del_status}\
            \n ❈╎الدردشه: {Redthon.chat.title}(`{Redthon.chat_id}`)",
        )
